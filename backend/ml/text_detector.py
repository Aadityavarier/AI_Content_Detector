...
class TextDetector:
    ...
    def _get_confidence_label(self, probability: float) -> str:
        ...
        elif probability >= 0.70: return "high"
        elif probability >= 0.55: return "moderate"
        elif probability >= 0.45: return "uncertain"
        else: return "low"
    ...
    def _run_inference(self, text: str) -> dict:
        ...
        avg = round(sum(ai_probs) / len(ai_probs), 4)
        return {
            "ai_probability": avg,
            "confidence": self._get_confidence_label(
                avg if avg >= 0.5 else 1 - avg
            ),
